...
const AddToCart = () => {
    ...
    const handleFinalizeCart = async () => {
        ...
        try {
            await axios.post('http://localhost:3500/cart/cartitems', cartData);
    ...
    const totalAmount = orderedFood.reduce((total, item) => total + (item.price * item.count), 0).toFixed(2);
        ...
        <>
                    ...
                    <table className="cart-table">
                        ...
                        <tbody>
                            {orderedFood.map((item, index) => {
                                ...
                                return (
                                    <tr key={index} style={postedItems.includes(item.name) ? styles.blurredRow : {}}>
                                        <td>{item.name}</td>
                                        <td>{item.price}</td>
                                        <td>
            ...
            >
                ...
                <div style={modalButtonContainer}>
                    <button style={modalButton} onClick={handleOrderMore}>Order More</button>
                    <button style={modalButton} onClick={handlePayBill}>Pay Bill</button>
                    <button style={modalButton} onClick={handleOrderStatus}>Check Order Status</button>
                </div>
            </Modal>
        </>
    );
